...
st.title("Økonomisk Politik Monitor")
...
with st.spinner("Henter events..."):
    ...
    ORDER BY Day DESC, Samlet_Mentions DESC
    LIMIT 50
    """
    ...
    df_events["Dato"] = pd.to_datetime(
        df_events["Dato"].astype(str), format="%Y%m%d", errors="coerce"
    ).dt.strftime("%d-%m-%Y")
    st.dataframe(
        df_events[
            [
